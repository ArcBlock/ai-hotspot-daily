...
export function findSkillRoot(startDir?: string): string {
  ...
  for (let i = 0; i < 10; i++) {
    try {
      if (Bun.file(resolve(dir, "sources/trending.yaml")).size) return dir;
    } catch {}
...
export async function writeOutput(output: SourceOutput, outputPath: string): Promise<void> {
...
export async function fetchWithTimeout(
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; HotspotInfograph/1.0)",
        ...options.headers,
      },
    });
    return response;
